...
async def test_signup_admin_domain(client: AsyncClient):
    res = await client.post("/auth/signup", json={
...
async def test_verify_email_correct_otp(client: AsyncClient):
    ...
    # Read OTP directly from Redis (simulates what the user receives in email)
    redis = get_redis()
    otp = await redis.get(f"otp:{payload['email']}")
